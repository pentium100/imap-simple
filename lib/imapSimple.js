 ...
 *
 * @param {object} nsPrefix
 * @param {function} [callback] Optional callback, receiving signature (err, boxes)
 * @returns {undefined|Promise} Returns a promise when no callback is specified, resolving to `boxes`
 */
ImapSimple.prototype.getBoxes = function(nsPrefix, callback){
    var self = this;

    if (callback) {
        return nodeify(self.getBoxes(nsPrefix), callback);
    }

    return new Promise(function (resolve, reject) {
        self.imap.getBoxes(nsPreFix, function (err, boxes) {
            if (err) {
                reject(err);
                return;
            }
